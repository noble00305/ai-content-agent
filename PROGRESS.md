...
### 2026-07-18 (세션 — 주간 사이클 W1)
- **헌법 §4 절차 실행** (주차 판정: W1, 7/13–7/19)
- **측정** (전부 실측, 브라우저 익스텐션 미연결 → curl + 디버그 크롬 Selenium 자동화 전용 탭으로 대체):
  - [티스토리] 발행 3편 공개 확인(7/13·15·17, 예약 2편 7/20·22 정상 대기, 7/17 글 댓글 1) / **다음 색인 3/3** (site: 검색에 /107·/108·/109 전부 노출) / 구글 색인 0/3 (site:URL 개별 검색 "결과 없음") / 유입: 신규 글 조회 1(무료 AI 도구), 검색 키워드 0, 최근 3일 리퍼러 bing 9·**chatgpt.com 2**·yahoo 1·google 1 / 제휴클릭 해당없음(링크 W3 삽입 예정)
  - [vercel 대조군] **구글 색인 0→11건**(홈+글 10편, site: num=100 실측) — 동결 상태에서 시간만으로 색인 시작, S-observe 가설 적중. 다음 0건. GSC 크롤 수는 익스텐션 미연결로 미확인(다음 사이클)
- **W1 잔여 임무 완료 — 티스토리 검색 노출 설정 확인**: 현 티스토리 설정엔 검색 노출 토글이 없음(관리>블로그 실측). RSS 전체 공개(50개)·sitemap.xml 자동 생성 정상 + 신규 3편 다음 실노출로 기능상 노출 정상 판정
- **기록**: strategy-board.json에 W1 스냅샷 append, K1 상태 "사실상 통과 궤도(다음 3/3)"로 갱신, learnings 3건 추가(다음 1~5일 색인 실증 / vercel 시간 경과 색인 / chatgpt.com 리퍼러)
- 예산 사용 ₩0/30,000. 이탈 없음. 승인 필요 없음(이번 주 공개게시·지출 없음)
- 다음: W2(7/20~) — ①예약 2편 발행 확인 ②색인 1차 측정(다음/구글) ③제휴 가입 신청(쿠팡 기존재 → Railway·ElevenLabs만, **계정 생성이라 사장님 승인 필요**) ④GSC로 vercel 크롤 수 확인

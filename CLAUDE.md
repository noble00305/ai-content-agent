...
## 팩트 무결성 규칙 (2026-06-23 추가 — 30편 점검에서 다수 위반 발견)
- **존재하지 않는 제품/버전 금지** — 학습 시점에 없는 미래 버전(GPT-5.5, Kimi K2.6, GLM-5.2 등)을 실재하는 양 단정하지 말 것. 모르면 검색해 확인, 안 되면 쓰지 말 것
- **가짜 정밀 수치 금지** — "HN 889점", "환각 3배", "1,445% 급증"처럼 검증 안 된 구체 수치를 권위 장치로 쓰지 말 것. HN 점수는 실제 토론 URL을 확인한 경우만 인용
- **실존 기관/논문에 가짜 숫자 부착 금지** — 맥킨지/가트너/Sensor Tower 등 진짜 출처에 지어낸 수치를 붙이면 가장 위험. 인용 수치는 실측 원문과 대조
- **출처/저자 오귀속 금지** — 보도 매체(기자)와 원저자(연구기관)를 구분할 것
- 검증 불가한 수치는 "추정", "범위"로 표기하거나 삭제
